...
public class JFormCheckList
{
    ...
    {
        log.info("JIRA Form (JFORM)");
        statusValidator(jFormsResponseResponse.getStatus());
        generalTabValidator(jFormsResponseResponse.getSolution(), jFormsResponseResponse.getProjectIdentifiers());
        designInputTabValidator(jFormsResponseResponse.getAddChangeSolutionCapabilities(),
                jFormsResponseResponse.getDevelopmentConsiderationsIncluded(), jFormsResponseResponse.getRequirements(),
                jFormsResponseResponse.getProjectTraceability(), jFormsResponseResponse.getChangesOrImpactToHazardAnalysis());
        designOutputTabValidator(jFormsResponseResponse.getTechnicalDesignDocumentation(),
                jFormsResponseResponse.getSourceCode());
    ...
    private void designInputTabValidator(String addChangeSolutionCapabilities, String developmentConsiderationsIncluded, String requirements, String projectTraceability, String changesOrImpactToHazardAnalysis)
    {
        if(!addChangeSolutionCapabilities.isEmpty())
        {
            ...
            {
                log.info("Adds/Changes Solution Capabilities: {}, a review of the Solution Record must attached to the JFORM.",
                        addChangeSolutionCapabilities);
            }
        }

        log.info("Have you implemented appropriate development considerations?: {}", developmentConsiderationsIncluded);
        ...
        else if(changesOrImpactToHazardAnalysis.equals("No"))
        {
            log.info("Changes or Impacts to Hazard Analysis: {}", changesOrImpactToHazardAnalysis);
            log.info("""
                    The "Hazard Analysis Location" field must include the location of the hazards (must be a valid Jazz link).
    ...
    private void generalTabValidator(List<String> solution, String projectIdentifiers)
    {
        ...
        {
            String[] solutionQueues = project[i].substring(1, project[i].length() - 1).split("[|]");
            String[] solutionQueue = new String[1];
            for(int j = 0; j < solutionQueues.length - 1; j++)
            {
                solutionQueue = solutionQueues[j].split("[-]");
            }
            log.info("Solution Queue is: {}", solutionQueue[0]);
            log.info("Solution link is: {}", solutionQueues[1]);
        }
    ...
    {
        log.info("Issue Status is: {}", jiraStatus);
    }
